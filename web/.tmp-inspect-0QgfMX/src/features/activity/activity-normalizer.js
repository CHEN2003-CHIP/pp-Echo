...
function buildActivitySummary(items = []) {
    ...
    return { status, eventCount, activityCount: items.length, toolCount, approvalCount, errorCount, startedAt, endedAt, durationLabel: startedAt && end ? (0, activity_utils_1.formatDurationMs)(Math.max(0, (end - startedAt) * 1000)) : "" };
}
...
function buildActivityItem(id, events) {
    ...
    const status = statuses.includes("error") ? "error" : statuses.includes("cancelled") ? "cancelled" : lastStatus === "success" || lastStatus === "warning" ? lastStatus : statuses.includes("running") ? "running" : statuses.includes("pending") ? "pending" : "success";
...
function buildStep(event, index, fallbackStartedAt) {
    ...
    return { id: (0, activity_utils_1.eventStableKey)(event, index), kind, label: narrative.title, detail: narrative.detail, narrative: narrative.body, timestamp: event.timestamp, startedAt, endedAt: terminalEndedAt, durationLabel, status, tone: status, attachments: toolResultAttachments(event.details || {}), rawType: event.type, safeRaw: (0, activity_utils_1.safeRawEvent)(event) };
}
...
function summarizeActivity(phase, events, entries) {
    const latest = [...events].reverse()[0];
    const title = narrativeHeadline(phase, latest);
    const body = [phaseNarrative(phase, events), entries.map((entry) => entry.narrative).filter((value) => Boolean(value)).slice(0, 3).join(" ")].filter(Boolean).join(" ").trim();
    return { title, summary: (0, activity_utils_1.truncateText)(body || title, 180), narrative: body || title };
}
function narrativeHeadline(phase, last) {
    if (phase === "preparing" || phase === "analyzing" || phase === "finalizing")
        return "正在推进这一步";
    if (phase === "planning")
        return "已经形成下一步计划";
    if (phase === "tool")
        return naturalToolLabel(last);
    if (phase === "approval")
        return "等待你确认关键操作";
    if (phase === "artifact")
        return "正在整理变更结果";
...
function phaseNarrative(phase, events) {
    const latest = [...events].reverse()[0];
    if (phase === "tool")
        return toolNarrative(latest);
    if (phase === "approval")
        return approvalNarrative(latest);
    if (phase === "planning")
        return "我已经把接下来要做的事情排好了顺序。";
    if (phase === "preparing")
        return "我先整理上下文，确认现在能安全处理哪些内容。";
    if (phase === "analyzing")
        return "我在看任务本身和当前工作区，尽量把方向摸清楚。";
    if (phase === "finalizing")
        return "我在收尾，把结果整理成更容易理解的回答。";
    if (phase === "checkpoint")
        return "我保留了一个检查点，方便后续回退或继续。";
    if (phase === "artifact")
        return "我把变更产物整理好了，等你确认下一步。";
    if (phase === "subagent")
        return "我把部分工作交给了子任务处理。";
    if (phase === "memory")
        return "我在整理可复用的信息，避免下次重复判断。";
    if (phase === "queue")
        return "我在调整接下来的执行顺序。";
    return "我正在推进这个任务。";
}
function naturalStepNarrative(event) {
    ...
    if (event.is_error || event.type === "tool_error" || event.type.endsWith("_error")) {
        const text = command ? `这条命令出了问题：${command}` : (0, activity_utils_1.firstString)(event.message, "这一步出了问题。");
        return { title: "运行失败", detail: text, body: text };
    }
    if (event.type === "before_provider_request")
        return { title: "准备模型请求", detail: "我先整理上下文，再发出下一次模型请求。", body: "我先整理上下文，再发出下一次模型请求。" };
    if (event.type === "provider_response")
        return { title: "收到模型回应", detail: "模型已经返回结果，我正在把它纳入当前过程。", body: "模型已经返回结果，我正在把它纳入当前过程。" };
    if (event.type === "reasoning_start")
        return { title: "开始分析", detail: "我先判断这次任务该怎么推进。", body: "我先判断这次任务该怎么推进。" };
    if (event.type === "reasoning_delta")
        return { title: "继续分析", detail: (0, activity_utils_1.truncateText)(String(event.delta || "我在继续思考下一步。"), 220), body: (0, activity_utils_1.truncateText)(String(event.delta || "我在继续思考下一步。"), 220) };
    if (event.type === "reasoning_summary")
        return { title: "整理结论", detail: (0, activity_utils_1.truncateText)((0, activity_utils_1.firstString)(details.summary, event.message, "我把当前想法整理了一下。"), 240), body: (0, activity_utils_1.truncateText)((0, activity_utils_1.firstString)(details.summary, event.message, "我把当前想法整理了一下。"), 240) };
    if (event.type === "reasoning_end")
        return { title: "准备回复", detail: "我已经把分析收束到一个可执行的结论。", body: "我已经把分析收束到一个可执行的结论。" };
    if (event.type.startsWith("planner_")) {
    ...
    if (event.type.startsWith("tool_")) {
        if (toolName === "run_shell" || command) {
            ...
            return { title: "运行命令", detail: shellText, body: shellText };
        }
        if (/(read|cat|open|get_file|file_read)/i.test(toolName)) {
            const text = path ? `我在查看 ${shortPath(path)}。` : "我在查看一个文件。";
            return { title: "查看文件", detail: text, body: text };
        }
        if (/(list|ls|tree|glob)/i.test(toolName))
            return { title: "查看结构", detail: "我先看一下项目结构，确认文件分布。", body: "我先看一下项目结构，确认文件分布。" };
        if (/(search|grep|rg|find)/i.test(toolName))
            return { title: "搜索内容", detail: "我在项目里搜索相关内容，尽量缩小范围。", body: "我在项目里搜索相关内容，尽量缩小范围。" };
        if (/(patch|apply_patch|edit|write|replace|create)/i.test(toolName)) {
            const text = path ? `我准备修改 ${shortPath(path)}。` : "我准备修改一些文件。";
            return { title: "修改文件", detail: text, body: text };
        }
        if (/web\.(news|search)/i.test(toolName))
            return { title: "搜索网页", detail: "我在网页上找补充信息。", body: "我在网页上找补充信息。" };
        if (/web\.(fetch|open)/i.test(toolName))
            return { title: "读取网页", detail: "我打开了一个网页内容。", body: "我打开了一个网页内容。" };
        return { title: "调用工具", detail: `我调用了 ${toolName || "一个工具"}。`, body: `我调用了 ${toolName || "一个工具"}。` };
    }
    if (event.type === "approval_result" || event.type.includes("approval"))
        return approvalNarrative(event);
    if (event.type.startsWith("checkpoint_") || event.type.startsWith("session_safe_rewind"))
        return { title: "记录检查点", detail: "我保留了一个安全的中间状态。", body: "我保留了一个安全的中间状态。" };
    if (event.type.startsWith("subagent_")) {
    ...
    if (event.type === "cancel_requested")
        return { title: "收到取消请求", detail: "当前任务准备停下来。", body: "当前任务准备停下来。" };
    if (event.is_error)
        return { title: "发生错误", detail: (0, activity_utils_1.truncateText)((0, activity_utils_1.firstString)(event.message, "这一步出了问题。"), 220), body: (0, activity_utils_1.truncateText)((0, activity_utils_1.firstString)(event.message, "这一步出了问题。"), 220) };
    return { title: event.type.replace(/_/g, " "), detail: (0, activity_utils_1.truncateText)((0, activity_utils_1.firstString)(event.message, "我在继续推进这个任务。"), 220), body: (0, activity_utils_1.truncateText)((0, activity_utils_1.firstString)(event.message, "我在继续推进这个任务。"), 220) };
}
...
function approvalNarrative(event) {
    const details = event.details || {};
    const actionType = (0, activity_utils_1.firstString)(details.action_type, event.tool_name);
    const command = (0, activity_utils_1.firstString)(details.command, event.tool_args?.command);
    const path = (0, activity_utils_1.firstString)(details.path, details.absolute_path, details.target_path);
    if (actionType === "run_shell" && command)
        return { title: "等待确认命令", detail: `我在等你确认这条命令：${command}`, body: `我在等你确认这条命令：${command}` };
    if ((actionType === "write_file" || actionType === "edit_file") && path)
        return { title: "等待确认修改", detail: `我已经准备好修改 ${shortPath(path)}，等你确认。`, body: `我已经准备好修改 ${shortPath(path)}，等你确认。` };
...
function toolNarrative(event) {
    const details = event?.details || {};
    const toolName = String(event?.tool_name || "");
    const command = (0, activity_utils_1.firstString)(details.command, event?.tool_args?.command);
    const path = (0, activity_utils_1.firstString)(details.path, details.absolute_path, details.target_path);
    if (toolName === "run_shell" || command)
        return command ? `我正在运行命令：${command}` : "我正在运行一条命令。";
    if (/(read|cat|open|get_file|file_read)/i.test(toolName))
        return path ? `我正在查看 ${shortPath(path)}。` : "我正在查看一个文件。";
    if (/(list|ls|tree|glob)/i.test(toolName))
        return "我正在查看项目结构。";
    if (/(search|grep|rg|find)/i.test(toolName))
        return "我正在搜索相关内容。";
    if (/(patch|apply_patch|edit|write|replace|create)/i.test(toolName))
        return path ? `我正在修改 ${shortPath(path)}。` : "我正在修改文件。";
