...
exports.MAX_VISIBLE_ATTACHMENTS = void 0;
...
exports.partitionVisibleAttachments = partitionVisibleAttachments;
exports.RichMessageContent = RichMessageContent;
exports.parseMarkdown = parseMarkdown;
exports.sanitizeMediaUrl = sanitizeMediaUrl;
...
function RichMarkdown({ text, className = "", streaming = false, plain = false }) {
    if (streaming) {
        return ((0, jsx_runtime_1.jsxs)("p", { className: className ? `md-paragraph md-streaming-plain ${className}` : "md-paragraph md-streaming-plain", children: [text, (0, jsx_runtime_1.jsx)("span", { className: "stream-cursor markdown-cursor" })] }));
    }
    if (!text) {
        return null;
    }
    return (0, jsx_runtime_1.jsx)("pre", { className: className ? `markdown md-large-plain ${className}` : "markdown md-large-plain", children: text });
}
function RichMessageAttachments({ attachments }) {
    if (!attachments.length)
        return null;
    const { visible, hiddenCount } = partitionVisibleAttachments(attachments);
    return ((0, jsx_runtime_1.jsxs)("div", { className: "attachment-grid", children: [visible.map((attachment, index) => {
                const label = attachment.alt || attachment.title || attachment.name || `Image ${index + 1}`;
                return ((0, jsx_runtime_1.jsxs)("figure", { className: "attachment-card", children: [(0, jsx_runtime_1.jsx)("a", { href: attachment.url, target: "_blank", rel: "noreferrer noopener", title: label, children: (0, jsx_runtime_1.jsx)("img", { src: attachment.url, alt: attachment.alt || label, loading: "lazy", referrerPolicy: "no-referrer" }) }), (0, jsx_runtime_1.jsxs)("figcaption", { children: [(0, jsx_runtime_1.jsx)("span", { children: label }), attachment.title && attachment.title !== label && (0, jsx_runtime_1.jsx)("small", { children: attachment.title })] })] }, `${attachment.url}-${index}`));
            }), hiddenCount > 0 ? ((0, jsx_runtime_1.jsx)("div", { className: "attachment-card attachment-overflow", children: (0, jsx_runtime_1.jsxs)("span", { children: [hiddenCount, " more attachment", hiddenCount === 1 ? "" : "s"] }) })) : null] }));
}
function partitionVisibleAttachments(attachments, limit = exports.MAX_VISIBLE_ATTACHMENTS) {
...
function RichMessageContent({ text, attachments, streaming = false, plain = false }) {
    return ((0, jsx_runtime_1.jsxs)("div", { className: "message-body", children: [(0, jsx_runtime_1.jsx)(RichMarkdown, { text: text, streaming: streaming, plain: plain }), !streaming && (0, jsx_runtime_1.jsx)(RichMessageAttachments, { attachments: attachments })] }));
}
...
function parseMarkdown(text) {
    ...
    while (index < lines.length && blocks.length < MAX_MARKDOWN_PARSE_BLOCKS && guard < lines.length * 4 + 20) {
        ...
        while (index < lines.length) {
            ...
            if (!current.trim())
                break;
            if (isBlockBoundary(current) && paragraphLines.length > 0)
                break;
...
function sanitizeMediaUrl(raw, options) {
    ...
    if (!value)
        return null;
    if (/^(data:|blob:)/i.test(value) && value.length > MAX_INLINE_MEDIA_URL_CHARS)
        return null;
...
function MarkdownImage({ url, alt, title }) {
    ...
    return ((0, jsx_runtime_1.jsxs)("figure", { className: "md-image", children: [(0, jsx_runtime_1.jsx)("a", { href: url, target: "_blank", rel: "noreferrer noopener", title: label, children: (0, jsx_runtime_1.jsx)("img", { src: url, alt: alt || label, loading: "lazy", referrerPolicy: "no-referrer", onError: () => setBroken(true) }) }), title || alt ? (0, jsx_runtime_1.jsx)("figcaption", { children: title || alt }) : null] }));
}
