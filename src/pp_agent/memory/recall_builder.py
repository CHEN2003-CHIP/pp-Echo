...
from pp_agent.memory.classification import (
    CATEGORY_LABELS,
    MemoryCategory,
...
class RecallSnippetBuilder:
    """Build a bounded [History Recall] prompt block from retrieved chunks."""
    ...
    def _priority_score(self, chunk: RetrievedChunk) -> float:
        ...
        if self.prioritize_long_term_preferences and self._category_for(chunk) == "preference":
            score += 1.5
        if looks_like_path_or_command(text):
            score += 1.0 * self.path_weight_boost
        if is_error_or_fix(text):
            score += 0.9
    ...
    def _header_lines() -> list[str]:
        return [
            "[History Recall]",
            "Relevant long-term memory snippets. Use them only when they help the current task:",
        ]
    ...
    def _summarize(self, chunk: RetrievedChunk) -> str:
        ...
        category = self._category_for(chunk)
        return f"[{category}] {text} ({location})"

    def _compress_error_text(self, text: str) -> str:
        if not is_error_or_fix(text.lower()):
            return text
        ...
        for line in lines:
            ...
            elif re.search(r"([a-z]:\\|/|src/|tests/|docs/|\.py\b|\.ts\b|\.json\b)", line, re.IGNORECASE):
                important.append(line)
            if len(important) >= 3:
                break
        if not important:
    ...
    def _categorize(self, chunk: RetrievedChunk) -> str:
        return CATEGORY_LABELS[self._category_for(chunk)]

    def _category_for(self, chunk: RetrievedChunk) -> MemoryCategory:
        metadata_category = str((chunk.metadata or {}).get("memory_category") or "").strip()
        if metadata_category in CATEGORY_LABELS:
            return metadata_category  # type: ignore[return-value]
