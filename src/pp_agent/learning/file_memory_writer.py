...
class FileMemoryWriter:
    ...
    def auto_apply(self, candidates: list[LearningCandidate]) -> list[FileMemoryWriteResult]:
        ...
        for candidate in candidates:
            ...
            wrote_memory = wrote_memory or result.action in {"bootstrap_memory", "detailed_memory"}
        if wrote_memory:
            self._sync_bootstrap_memory()
            if self.settings.detailed_memory_sync_index_after_write:
    ...
    def apply_candidate(self, candidate: LearningCandidate, *, auto: bool = False) -> FileMemoryWriteResult:
        ...
        if auto and not self._meets_auto_confidence(candidate):
        ...
        if target == "detailed_memory" and self.settings.detailed_memory_enable:
        ...
        self.store.append_project_memory(self.curator.memory_entry(candidate))
        self._compact_project_memory_if_needed()
        self._sync_bootstrap_memory()
        self.store.update(candidate.mark_applied())
        return FileMemoryWriteResult(candidate_id=candidate.id, action="bootstrap_memory", path=self.workspace / "MEMORY.md")

    def _resolve_target(self, candidate: LearningCandidate) -> str:
        if candidate.suggested_target in {"bootstrap_memory", "detailed_memory"}:
    ...
    def _detailed_memory_path(self, candidate: LearningCandidate) -> Path:
        ...
        if candidate.kind == "user_preference":
            name = "preferences.md"
        elif any(marker in text for marker in ("architecture", "design", "decision", "架构", "设计", "决策")):
            name = "architecture.md"
        elif any(marker in text for marker in ("bug", "fix", "failed", "failure", "error", "traceback", "修复", "错误")):
            name = "bugs.md"
        elif any(marker in text for marker in ("debug", "investigate", "diagnose", "调试", "排查")):
            name = "debugging.md"
        elif candidate.kind == "workflow" or any(marker in text for marker in ("workflow", "procedure", "步骤")):
            name = "workflows.md"
        else:
            name = "lessons.md"
    ...
    def _sync_file_memory_index(self, results: list[FileMemoryWriteResult]) -> None:
        try:
            ...
            for index, result in enumerate(results):
                if result.action in {"bootstrap_memory", "detailed_memory"}:
