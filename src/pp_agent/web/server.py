...
def create_app(workspace: Path, *, manager: Optional[WebSessionManager] = None):
    try:
    ...
    session_manager = manager or WebSessionManager(workspace)
    ...
    @app.get("/api/health")
    def health() -> dict:
        return {"ok": True, "app": "pp-Echo", "workspace": str(workspace)}
    ...
    def settings() -> dict:
        loaded = bootstrap.load_settings(workspace)
    ...
    def session_tree(session_id: str) -> dict:
        try:
            return sdk.get_session_tree(workspace, session_id=session_id)
        except FileNotFoundError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc
    ...
    def fork(session_id: str, head_id: Optional[str] = None) -> dict:
        try:
            return sdk.fork_session(workspace, session_id, head_id=head_id)
        except FileNotFoundError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc

    @app.get("/api/sessions/{session_id}/checkpoints")
    def checkpoints(session_id: str) -> dict:
        return {"checkpoints": sdk.list_checkpoints(workspace, session_id=session_id)}
    ...
    def reject_pending_action(token: str) -> dict:
        try:
            return reject_pending_action_by_token(workspace, token, render=False)
        except (FileNotFoundError, RuntimeError, ValueError, PermissionError) as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc

    @app.get("/api/capabilities")
    def capabilities(kind: Optional[str] = None, include_mcp: Optional[bool] = None) -> dict:
        return {"capabilities": sdk.list_capabilities(workspace, kind=kind, include_mcp=include_mcp)}
